    ...
    <div class="space-y-6">
        @foreach ($tugases->subtugas as $subtugas)
            ...
            <div class="w-full p-8 rounded-xl border-b border-b-hijau bg-white flex items-center justify-between">
                ...
                <a href="{{ $userFinish ? route('subtugas.edit', $subtugas->id) : route('subtugas.show', $subtugas->id) }}"
                    class="py-2 px-8 rounded-xl bg-hijau text-white text-lg font-semibold">{{ $userFinish ? 'Lihat' : 'Kerjakan' }}</a>
            </div>
        @endforeach
