...
<div class="space-y-6">
    <div class="border border-abu-400 rounded-xl p-5">
        ...
        @if ($answers->file)
            <embed class="block rounded-xl w-full h-screen bg-[#D9D9D9] my-6" src="{{ asset('storage/TugasAnswer/file/' . $answers->file) }}" type="application/pdf">
            </embed>
        @else
...
<div class="w-full">
    <button data-modal-target="default-modal" data-modal-toggle="default-modal" type="button" class="p-4 rounded-xl bg-hijau text-lg text-white font-semibold mt-16 block w-fit mx-auto">Feedback</button>
</div>
            ...
            @method('PUT')
                <div class="p-4 md:p-5 space-y-9">
                    <div class="flex flex-col gap-8">
                        <textarea name="feedback" id="feedback" rows="6" value="{{ $answers->feedback }}" placeholder="Masukkan Feedback" class="w-full px-4 py-2 border-2 outline-none border-hijau-400 rounded-xl focus:border-hijau focus:border-2">{{ $answers->feedback }}</textarea>
                    </div>
                </div>
                <!-- Modal footer -->
