...
@section('content')
    ...
    <div class="space-y-6">
        @foreach ($users as $user)
            @php
                $userAnswers = $user->tugas_answer->where('subtugas.tugas.id', $tugases->id);
                $subtugasLength = $subtugases->count();
                $answerLength = $userAnswers->count();
                $answerPercentage = $subtugasLength > 0 ? ($answerLength / $subtugasLength) * 100 : 0;
                $userGrade = $tugases->tugas_nilai->where('murid_id', $user->id)->first();
            @endphp
