...
@section('content')
    ...
    @if (count($tugases->subtugas) == 0)
        <div id="add-submateri" class="flex justify-between items-center">
            ...
            <button type="button" onclick="showSectionSubmateri()" class="text-hijau underline">+ Hapus tugas</button>
        </div>
    @else
        ...
        <div class="my-8">
            ...
            <div class="flex justify-center">
                <button type="button" onclick="showSectionSubmateri()"
                    class="text-[1.75rem] text-white size-14 rounded-full bg-hijau-400">+</button>
            </div>
        </div>
    @endif
    ...
    <div id="section-submateri" class="hidden my-6 p-8 rounded-xl bg-white border-b border-hijau">
        ...
        <form method="POST" action="{{ route('subtugas-guru.store') }}">
            ...
            <div class="p-4 md:p-5 space-y-9">
                ...
                <input type="text" id="tahap" name="tahap" value="{{ old('tahap') }}"
                    placeholder="Masukkan Nama Tahap" required autofocus
                    class="w-full px-4 py-2 border-2 outline-none border-hijau-400 rounded-xl focus:border-hijau focus:border-2">
                <textarea name="deskripsi" id="deskripsi" rows="6" value="{{ old('deskripsi') }}" placeholder="Masukkan Deskripsi"
            ...
            <div class="flex items-center p-4 md:p-5 rounded-b gap-4 justify-end w-full">
                <button type="button" onclick="closeSectionSubmateri()"
