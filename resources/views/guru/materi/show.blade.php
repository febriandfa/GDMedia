...
<div class="flex justify-between items-center">
    ...
    <div class="flex items-center gap-3">
        <button data-modal-target="default-modal-edit" data-modal-toggle="default-modal-edit" type="button" class="text-[1.75rem] text-white">
        ...
        <button data-modal-target="default-modal-delete" data-modal-toggle="default-modal-delete" type="button" class="text-[1.75rem] text-white">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 35 35" fill="none">
        ...
        <div class="relative bg-white rounded-lg shadow">
            <div class="flex items-center justify-between p-4 md:p-5 rounded-t">
                <h3 class="text-xl font-semibold">
                    Edit Elemen Pembelajaran
                </h3>
                <button type="button" class="bg-transparent rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-hide="default-modal-edit">
            ...
            @method('PUT')
                ...
                <div class="flex items-center p-4 md:p-5 rounded-b gap-4 justify-end w-full">
                    <button data-modal-hide="default-modal-edit" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:z-10 focus:ring-4 focus:ring-gray-100">Tutup</button>
                    <button data-modal-hide="default-modal-edit" type="submit" class="text-white bg-hijau hover:bg-hijau-400 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Simpan</button>
                </div>
            </form>
        </div>
    </div>
...
{{-- Modal Delete --}}
<div id="default-modal-delete" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-2xl max-h-full">
        <div class="relative bg-white rounded-lg shadow">
            <div class="flex items-center justify-end p-4 md:p-5 rounded-t">
                <button type="button" class="bg-transparent rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-hide="default-modal-delete">
                    <svg class="w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                    </svg>
                    <span class="sr-only">Close modal</span>
            ...
            <div class="w-full px-5">
                <div class="w-full h-full rounded-xl border border-abu-500 p-7 flex flex-col justify-center items-center gap-2">
                    ...
                    <p class="font-semibold text-xl">Hapus Elemen dan Materinya?</p>
                </div>
            </div>
            {{-- Content End Here --}}
            <div class="flex items-center p-4 md:p-5 rounded-b gap-4 justify-end w-full">
                <button data-modal-hide="default-modal-delete" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:z-10 focus:ring-4 focus:ring-gray-100">Tutup</button>
                <form method="POST" action="{{ route('materi-guru.destroy', $materis->id) }}">
                @csrf
                @method('DELETE')
...
{{-- Add Submateri and List Submateri Section --}}
@if (count($materis->submateri) == 0)
    <div id="add-submateri" class="flex justify-between items-center my-12">
        <div class="flex items-center gap-4">
            ...
            <p class="text-xl font-semibold">Belum ada isi kegiatan pembelajaran</p>
        </div>
...
{{-- Tambah Submateri --}}
<div id="section-submateri" class="hidden p-8 rounded-xl bg-white border-b border-hijau">
    <h3 class="text-xl font-semibold">
        Materi Pembelajaran
    </h3>
    <form method="POST" action="{{ route('submateri-guru.store') }}" enctype="multipart/form-data">
    @csrf
        <div class="p-4 md:p-5 space-y-9">
            <input type="text" id="materi_id" name="materi_id" value="{{ $materis->id }}" class="hidden">
            <input type="text" id="nama" name="nama" value="{{ old('nama') }}" placeholder="Masukkan Nama Elemen" required autofocus class="w-full px-4 py-2 border-2 outline-none border-hijau-400 rounded-xl focus:border-hijau focus:border-2">
